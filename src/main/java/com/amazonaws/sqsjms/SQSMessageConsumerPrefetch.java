...
import com.amazonaws.sqsjms.util.ExponentialBackoffStrategy;
...
public class SQSMessageConsumerPrefetch implements Runnable, PrefetchManager {
    ...
    SQSMessageConsumerPrefetch(SQSSessionCallbackScheduler sqsSessionRunnable, Acknowledger acknowledger,
                               NegativeAcknowledger negativeAcknowledger, SQSDestination sqsDestination,
                               AmazonSQSClientJMSWrapper amazonSQSClient, int numberOfMessagesToPrefetch) {
            ...
            try {
                ...
                synchronized (stateLock) {
                    ...
                    prefetchBatchSize = Math.min(
                            (numberOfMessagesToPrefetch - messagesPrefetched), SQSJMSClientConstants.MAX_BATCH);
                }
